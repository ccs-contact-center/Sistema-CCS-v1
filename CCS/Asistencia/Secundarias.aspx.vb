...
Public Class Secundarias
    ...
    Sub ListaAgentes()
        ...
        Dim strConnString As String = ConfigurationManager.ConnectionStrings("db").ConnectionString
        Dim strQuery As String = "SELECT a.id, a.nombres+ ' ' + a.paterno+ ' ' +a.materno as Nombre, b.nombres+' '+b.paterno as Supervisor, c.campania as Campaña FROM SYS_empleados a LEFT JOIN SYS_empleados b ON a.jefe_directo = b.id LEFT JOIN SYS_campanias c ON a.campaña = c.id WHERE (a.status = 2 OR a.status = 7) AND a.puesto = 0 AND a.campaña = '" & DropDownList1.SelectedItem.Value & "' ORDER BY campania"
        Dim con As New SqlConnection(strConnString)
        Dim cmd As New SqlCommand()
    ...
    Protected Sub ImageButton1_Click(sender As Object, e As ImageClickEventArgs) Handles ImageButton1.Click
        ...
        Dim strConnString As String = ConfigurationManager.ConnectionStrings("db").ConnectionString
        Dim strQuery As String = "UPDATE SYS_empleados SET curso = '" & DropDownList2.SelectedValue.ToString & "', status= 8 WHERE id = " & Session("seleccionsinasignar")
        Dim con As New SqlConnection(strConnString)
        Dim cmd As New SqlCommand()
        cmd.CommandType = CommandType.Text
        cmd.CommandText = strQuery
    ...
    Protected Sub ImageButton2_Click(sender As Object, e As ImageClickEventArgs) Handles ImageButton2.Click
        ...
        Dim strConnString As String = ConfigurationManager.ConnectionStrings("db").ConnectionString
        Dim strQuery As String = "UPDATE SYS_empleados SET curso = NULL, status = 2 WHERE id = " & Session("seleccionsupervisor")
        Dim con As New SqlConnection(strConnString)
        Dim cmd As New SqlCommand()
        cmd.CommandType = CommandType.Text
        cmd.CommandText = strQuery
